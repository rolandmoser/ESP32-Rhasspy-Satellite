...
if [ "$1" = "" ]; then
  ...
  if test -f "$FIRMWARE"; then
    ...
    echo ""
    tar cf - *.bin | ssh pi@$1 'tar xf - -C /tmp;sudo voice_esp32_reset;sudo voice_esptool --chip esp32 --port /dev/ttyAMA0 --baud 1500000 --before default_reset --after hard_reset write_flash -u --flash_mode dio --flash_freq 40m --flash_size detect 0x1000 /tmp/bootloader.bin 0x10000 /tmp/firmware.bin 0x8000 /tmp/partitions.bin'
    echo "done"
    echo ""
